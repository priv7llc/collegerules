CREATE OR REPLACE FUNCTION public.match_scholarships_for_user(_user_id uuid)
 RETURNS TABLE(id uuid, name text, sponsor text, amount_cents integer, deadline date, description text, eligibility_criteria jsonb, essay_prompts jsonb, external_url text, active boolean, created_at timestamp with time zone, updated_at timestamp with time zone, match_score integer, match_reasons text[])
 LANGUAGE plpgsql
