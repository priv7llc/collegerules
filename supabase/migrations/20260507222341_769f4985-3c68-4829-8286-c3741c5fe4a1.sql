...
ALTER TABLE public.scholarship_candidates
  ADD COLUMN IF NOT EXISTS discovered_for_user_id uuid REFERENCES auth.users(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_scholarship_candidates_discovered_for_user
  ON public.scholarship_candidates(discovered_for_user_id, status);

DROP POLICY IF EXISTS "Users can view own pending personal candidates" ON public.scholarship_candidates;
CREATE POLICY "Users can view own pending personal candidates"
  ON public.scholarship_candidates FOR SELECT
  USING (discovered_for_user_id = auth.uid() AND status = 'pending');

CREATE TABLE IF NOT EXISTS public.user_discovery_runs (
...
ALTER TABLE public.user_discovery_runs ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "users see own runs" ON public.user_discovery_runs;
CREATE POLICY "users see own runs" ON public.user_discovery_runs FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "admins see all runs" ON public.user_discovery_runs;
CREATE POLICY "admins see all runs" ON public.user_discovery_runs FOR SELECT
  USING (has_role(auth.uid(), 'admin'::app_role));

CREATE INDEX IF NOT EXISTS idx_user_discovery_runs_user_time
  ON public.user_discovery_runs(user_id, ran_at DESC);

CREATE OR REPLACE FUNCTION public.user_discovery_runs_today(_user_id uuid)
RETURNS integer LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public
AS $$
  SELECT COUNT(*)::integer FROM public.user_discovery_runs
  WHERE user_id = _user_id AND ran_at > now() - interval '24 hours';
$$;
...
BEGIN
  ...
  FOR cand IN
    ...
    ORDER BY relevance_score DESC NULLS LAST, deadline ASC NULLS LAST
  LOOP
    id := cand.id;
    name := cand.name;
